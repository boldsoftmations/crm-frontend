...
export const ProductForecastView = () => {
  ...
  const getAllProductionForecastDetails = async () => {
    try {
      setOpen(true);
      if (currentPage) {
        const response = await CustomerServices.getProductForecastPaginateData(
          currentPage
        );
        ...
        const response = await CustomerServices.getProductForecast();
        setProductForecast(response.data.results);
        const total = response.data.count;
        setpageCount(Math.ceil(total / 25));
      }
  ...
  const getSearchData = async (value) => {
    try {
      ...
      const response = await CustomerServices.getAllSearchProductForecast(
        filterQuery,
        filterSearch
      );
      if (response) {
  ...
  const handlePageClick = async (event, value) => {
    try {
      ...
      if (searchQuery) {
        const response = await CustomerServices.getAllProductForecastPaginate(
          page,
          filterQuery,
          searchQuery
        );
  ...
  const data = productForecast.map((row) => {
    const obj = {
      company: row.company,
